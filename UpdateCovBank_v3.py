...
class CovBankDB:
    ...
    def GetPrelevementToInsert(self,tsp_geo_match_df,envois_genome_qc,patient_id):
        ...
        BiobankId = envois_genome_qc['ID_Ref']
        ...
        return(tuple(map(GetVal,(patient_id,code_hopital,nom_hopital,adresse_hopital,date_prelev,BiobankId,date_envois_genome_quebec,travel_history,ct,outbreak,deb_voy1,fin_voy1,dest_voy1,deb_voy2,fin_voy2,dest_voy2,deb_voy3,fin_voy3,dest_voy3,deb_voy4,fin_voy4,dest_voy4,deb_voy5,fin_voy5,dest_voy5,deb_voy6,fin_voy6,dest_voy6,deb_voy7,fin_voy7,dest_voy7,deb_voy8,fin_voy8,dest_voy8,deb_voy9,fin_voy9,dest_voy9,deb_voy10,fin_voy10,dest_voy10,deb_voy11,fin_voy11,dest_voy11,deb_voy12,fin_voy12,dest_voy12,deb_voy13,fin_voy13,dest_voy13,genome_quebec_requete,BiobankId))))
...
class EnvoisGenomeQuebecData:
    ...
    def Format(self):
        self.pd_df = self.pd_df.loc[~self.pd_df['ID_Ref'].str.contains('^LSPQ-',regex=True),:]
        self.pd_df['Tableau3.Nom'] = self.pd_df['Tableau3.Nom'].str.replace('é','e').str.replace('è','e').str.replace('ç','c').str.replace('-','').str.replace(' ','')
        self.pd_df['Tableau3.Prénom'] = self.pd_df['Tableau3.Prénom'].str.replace('é','e').str.replace('è','e').str.replace('ç','c').str.replace('-','').str.replace(' ','')
        self.pd_df['Tableau3.NAM'] = self.pd_df['Tableau3.NAM'].str.replace('é','e').str.replace('è','e').str.replace('ç','c').str.replace("'","")
        self.pd_df['Tableau3.# Requête_ref'] = self.pd_df['Tableau3.# Requête_ref'].str.replace(' ','')
        self.pd_df['ID_Ref'] = self.pd_df['ID_Ref'].str.replace(' ','')

        self.pd_df['Tableau3.Nom'] = self.pd_df['Tableau3.Nom'].str.strip(' ')
        self.pd_df['Tableau3.Prénom'] = self.pd_df['Tableau3.Prénom'].str.strip(' ')
        self.pd_df['Tableau3.NAM'] = self.pd_df['Tableau3.NAM'].str.strip(' ')

        self.pd_df['Tableau3.Nom'] = self.pd_df['Tableau3.Nom'].str.upper()
        self.pd_df['Tableau3.Prénom'] = self.pd_df['Tableau3.Prénom'].str.upper()
        self.pd_df['Tableau3.NAM'] = self.pd_df['Tableau3.NAM'].str.upper()
        self.pd_df['Tableau3.NAM'] = self.pd_df['Tableau3.NAM'].str.replace(r'^0{1,}','')

        self.pd_df['Tableau3.Nom'].str.replace('É','E').str.replace('È','E').str.replace(',','').str.replace("*","").str.replace("(","").str.replace(")","").str.replace("&","").str.replace(".","").str.replace(",","").str.replace("'","")
        self.pd_df['Tableau3.Prénom'].str.replace('É','E').str.replace('È','E').str.replace(',','').str.replace("*","").str.replace("(","").str.replace(")","").str.replace("&","").str.replace(".","").str.replace(",","").str.replace("'","")

        self.pd_df['Tableau3.Date de naissance'] = pd.to_datetime(self.pd_df['Tableau3.Date de naissance'],format='%Y-%m-%d',errors='coerce')
        self.pd_df['Tableau3.Date de prélèvement'] = pd.to_datetime(self.pd_df['Tableau3.Date de prélèvement'],format='%Y-%m-%d',errors='coerce')
        self.pd_df['Tableau3.DateEnvoiGenomeQuebec'] = pd.to_datetime(self.pd_df['Tableau3.DateEnvoiGenomeQuebec'],format='%Y-%m-%d',errors='coerce')
        
        self.pd_df = self.pd_df.dropna(subset = ['Tableau3.Date de prélèvement'])

        self.pd_df = self.pd_df.rename(columns={'Tableau3.Nom':'Nom','Tableau3.Prénom':'Prénom','Tableau3.NAM':'NAM','Tableau3.# Requête_ref':'# Requête','Tableau3.Date de naissance':'Date de naissance','Tableau3.Date de prélèvement':'Date de prélèvement','Tableau3.DateEnvoiGenomeQuebec':'DateEnvoiGenomeQuebec'})

        #print(self.pd_df[['ID_Ref']])

    def ExtractOutbreakSample(self):
        outbreak_df = self.outbreak_obj_v2.GetNewOutbreakDf()
        #merge_df = pd.merge(outbreak_df,self.pd_df,left_on='COVBANK',right_on='# Requête',how='inner')
        merge_df = pd.merge(outbreak_df,self.pd_df,left_on='BIOBANK',right_on='ID_Ref',how='inner')
