...
Ajoute NAM DANS TABLE PATIENTS
...
'ID_Phylo':'ID_PHYLO'}
    ...
    def ToUpperCase(self):
        self.pd_df['NOMINFO'] = self.pd_df['NOMINFO'].str.replace('é','e').str.replace('è','e').str.replace('ç','c')
        self.pd_df['NOMINFO'] = self.pd_df['NOMINFO'].str.upper()

        self.pd_df['PRENOMINFO'] = self.pd_df['PRENOMINFO'].str.replace('é','e').str.replace('è','e').str.replace('ç','c')
...
'Date de naissance':'DTNAISSINFO','Date de prélèvement':'DATE_PRELEV','# Boîte':'NO_BOITE','DateEnvoiGenomeQuebec':'DATE_ENVOI_GENOME_QUEBEC','NAM':'NAM'}
    ...
    def RemoveWrongDateFormatRecord(self):
        ...
        for index, row in self.pd_df.loc[:,].iterrows():
            ...
            if((Utils.CheckDateFormat(str(type(row['DTNAISSINFO'])),row['DTNAISSINFO'],'DTNAISSINFO')) and (Utils.CheckDateFormat(str(type(row['DATE_PRELEV'])),row['DATE_PRELEV'],'DATE_PRELEV')) and (Utils.CheckDateFormat(str(type(row['DATE_ENVOI_GENOME_QUEBEC'])),row['DATE_ENVOI_GENOME_QUEBEC'],'DATE_ENVOI_GENOME_QUEBEC'))):
...
class SGILdata:
    ...
    def RemoveWrongDateFormatRecord(self):
        ...
        for index, row in self.pd_df.loc[:,].iterrows():
            ...
            if((Utils.CheckDateFormat(str(type(row['SAMPLED_DATE'])),row['SAMPLED_DATE'],'SAMPLED_DATE')) and (Utils.CheckDateFormat(str(type(row['DATE_NAISS'])),row['DATE_NAISS'],'DATE_NAISS'))):
                self.final_pd_df.loc[index_good] = row
                index_good += 1
            else:
                self.pd_df_with_wrong_date_format.loc[index_bad] = row
...
class MySQLcovid19Updator:
    ...
    def GetDSPmatch(self,nom,prenom,dt_naiss,dt_prelev,ch_dsp_code,use_ch,ch_lspq_code,ch_name,nam):
        return self.envois_genome_quebec_2_dsp_dat_matcher.GetDSPmatch(nom,prenom,dt_naiss,dt_prelev,ch_dsp_code,use_ch,ch_lspq_code,ch_name,nam)


    def GetDSPmatchWithSGILdat(self,nom,prenom,dt_naiss,dt_prelev,ch,numero_sgil,nam):
        return self.sgil_dat_2_dsp_dat_matcher.GetDSPmatch(nom,prenom,dt_naiss,dt_prelev,ch,numero_sgil,nam)
    ...
    def Insert(self,use_ch):
        ...
        for index,row in self.envois_gq.GetPandaDataFrame().loc[:,].iterrows():
            ...
            try:
                ...
                dsp_dat_match_rec =  self.GetDSPmatch(row['NOMINFO'],row['PRENOMINFO'],row['DTNAISSINFO'],row['DATE_PRELEV'],ch_dsp_code,use_ch,row['CODE_HOPITAL_LSPQ'],ch_name,row['NAM'])
        ...
        for index,row in self.sgil_dat.GetPandaDataFrame().loc[:,].iterrows():
            try:
                dsp_dat_match_rec = self.GetDSPmatchWithSGILdat(row['NOM'],row['PRENOM'],row['DATE_NAISS'],row['SAMPLED_DATE'],'LSPQ',row['NUMERO_SGIL'],row['NAM'])
...
class Utils:
    ...
    @staticmethod
    def CheckDateFormat(value,mydate,column):
    ...
    def GetDateFromStrDate(str_date):
        ...
        try:
            ...
            try:
                date_as_date = datetime.datetime.strptime(str_date.strip(' '),'%Y-%m-%d')
            except:
                #print(sys.exc_info())
                return None
        
        if(date_as_date.year > datetime.datetime.today().year):
            return None
        return date_as_date
...
class SGILdata_2_DSPdata_Matcher():
    ...
    def GetDSPmatch(self,nom,prenom,dt_naiss,dt_prelev,ch,numero_sgil,nam):
        try:
            nom = nom.strip(' ')
        ...
        try:
            ...
            if(len(str(nam).strip(' ')) > 9):
                match_df = dsp_dat_df.loc[dsp_dat_df['NAM_LSPQ'].str.strip(' ') == str(nam).strip(' '),:].copy()
...
class EnvoisGenomeQuebec_2_DSPdata_Matcher():
    def __init__(self,dsp_dat,excel_manager):
        self.dsp_dat = dsp_dat
        self.excel_manager = excel_manager
        self.no_match_df = pd.DataFrame(columns=['NOMINFO','PRENOMINFO','DTNAISSINFO','CODE_HOPITAL_DSP','CODE_HOPITAL_LSPQ','NOM_HOPITAL'])
    ...
    def GetDSPmatch(self,nom,prenom,dt_naiss,dt_prelev,dsp_ch_code,use_ch,lspq_ch_code,nom_hopital,nam):
        try: 
            nom = nom.strip(' ')
            nom_no_space = re.sub(r' |-','',nom)
            prenom = prenom.strip(' ')
            ...
            dsp_ch_code = dsp_ch_code.strip(' ')
                ...
                self.no_match_df.loc[self.nb_no_match] = {'NOMINFO':nom,'PRENOMINFO':prenom,'DTNAISSINFO':dt_naiss,'CODE_HOPITAL_DSP':dsp_ch_code,'CODE_HOPITAL_LSPQ':lspq_ch_code,'NOM_HOPITAL':nom_hopital}
                self.nb_no_match += 1
                return pd.DataFrame()
...
class ExcelManager:
    ...
    def SetFilePath(self):
        ...
        if self._debug:
            ...
            self.envois_genome_quebec_file = os.path.join(self.basedir_envois_genome_quebec,'ListeEnvoisGenomeQuebec_2020-09-28_corrCG.xlsx')
    ...
    def ReadEnvoisGenomeQuebecFile(self):
        ...
        return pd.read_excel(self.envois_genome_quebec_file,sheet_name=0)
